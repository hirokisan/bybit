...
type LinearConditionalOrderParam struct {
	Symbol         SymbolUSDT  `json:"symbol"`
	Side           Side        `json:"side"`
	OrderType      OrderType   `json:"order_type"`
	Price          float64     `json:"price"`
...
type LinearConditionalOrderResponse struct {
	CommonResponse `json:",inline"`
...
// LinearConditionalOrder : Cancel all active orders that are unfilled or partially filled. Fully filled orders cannot be cancelled.
func (s *AccountService) LinearConditionalOrder(param LinearConditionalOrderParam) (*LinearConditionalOrderResponse, error) {
	var res LinearConditionalOrderResponse

	body, err := json.Marshal(param)
	if err != nil {
		return &res, fmt.Errorf("json marshal for LinearConditionalOrder: %w", err)
	}
